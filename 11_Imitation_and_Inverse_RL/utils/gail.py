...
class GAIL(Module):
    def __init__(self, state_dim, action_dim, discrete, train_config=None) -> None:
        ...
        self.pi = PolicyNetwork(state_dim, action_dim, discrete)
        self.v = ValueNetwork(state_dim)
        self.d = Discriminator(state_dim, action_dim, discrete)
    ...
    def train(self, env, expert, render=False):
        ...
        for i in range(num_iters):
            ...
            while steps < num_steps_per_iter:
                ep_obs, ep_acts, ep_rwds, ep_costs, ep_disc_costs = [], [], [], [], []
                ep_gms, ep_lmbs = [], []
                t, done = 0, False
                ...
                next_vals = torch.cat((curr_vals[1:], FloatTensor([[0.]]))).detach()
            ...
            print(f"Iterations: {i+1},   Reward Mean: {np.mean(rwd_iter)}")
            ...
            opt_d.zero_grad()
            loss = (
                torch.nn.functional.binary_cross_entropy_with_logits(exp_scores, torch.zeros_like(exp_scores)) +
            ...
            def Hv(v):
                return get_flat_grads(torch.dot(grad_diff, v), self.v).detach()
            ...
            grad_disc_causal_entropy = get_flat_grads(disc_causal_entropy.mean(), self.pi)
